...
def build_generation_prompt_from_outline(
    outline: ChapterOutline,
    writing_style: str,
    story_context: dict,
    domain_spec: str,
    ...
    基于结构化骨架构建生成提示词
    """
    from ai_handler import _brief_style_for_generation

    style_brief = _brief_style_for_generation(writing_style)

    scene_instructions = []
    for i, scene in enumerate(outline.scenes):
        node_type_val = scene.node_type.value if hasattr(scene.node_type, "value") else scene.node_type
        lines = [f"## 场景 {i+1} ({node_type_val})"]
        lines.append(f"- 功能：{scene.purpose}")
        if scene.emotional_beat:
            emotion_val = scene.emotional_beat.value if hasattr(scene.emotional_beat, "value") else scene.emotional_beat
            lines.append(f"- 情绪：{emotion_val}")
        if scene.location:
            lines.append(f"- 地点：{scene.location}")
        if scene.characters:
            lines.append(f"- 角色：{', '.join(scene.characters)}")
        if scene.content_summary:
            lines.append(f"- 摘要：{scene.content_summary}")
        if scene.is_optional:
            lines.append(f"- 可选：是")
        scene_instructions.append("\n".join(lines))

    prompt = f"""# 第 {chapter_number} 章 - 同结构改编
...
{chr(10)}{chr(10).join(scene_instructions)}

## 因果链条
{chr(10)}{chr(10).join(f'- {item}' for item in outline.causal_chain[:10]) if outline.causal_chain else '（无特定因果链条）'}

## 必须保留的事实
{chr(10)}{chr(10).join(f'- {item}' for item in outline.must_keep_facts[:10]) if outline.must_keep_facts else '无'}

## 本章结尾状态
{outline.ending_state or '（无特定要求）'}

## 为下一章留的钩子
{outline.hook_for_next or '（无特定要求）'}

## 生成要求
1. 按场景顺序生成，每个场景完成它的功能
2. 保持风格一致，但不要抄参考原文的句子
3. 字数大约 {target_length} 字
4. 以 '# 第{chapter_number}章 标题' 开头
"""
    return prompt
